...
static void doTreeSearchBetween(Tree t, Node n, Record lower,
                                Record upper, List l) {
    int lowerRec = RecordGetZid(lower);
    int upperRec = RecordGetZid(upper);

    if (n == NULL) {
        return;
    }
    if (RecordGetZid(n->rec) > lowerRec) {
        doTreeSearchBetween(t, n->left, lower, upper, l);
    }
    if (RecordGetZid(n->rec) >= lowerRec && RecordGetZid(n->rec) <= upperRec) {
        ListAppend(l, n->rec);
    }
